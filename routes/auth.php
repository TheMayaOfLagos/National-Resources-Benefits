...
Route::middleware('auth')->group(function () {
    // Login OTP Routes (before other auth checks)
    Route::prefix('login-otp')->name('auth.login-otp.')->group(function () {
        Route::get('/', [LoginOtpController::class, 'show'])->name('show');
        Route::post('/verify', [LoginOtpController::class, 'verify'])->name('verify');
        Route::post('/resend', [LoginOtpController::class, 'resend'])->name('resend');
    });
    ...
    Route::prefix('idme')->name('auth.idme.')->group(function () {
        Route::get('/verify', [\App\Http\Controllers\Auth\IdMeVerificationController::class, 'show'])
            ->name('verify');
        Route::get('/redirect', [\App\Http\Controllers\Auth\IdMeVerificationController::class, 'redirect'])
            ->name('redirect');
        Route::get('/callback', [\App\Http\Controllers\Auth\IdMeVerificationController::class, 'callback'])
            ->name('callback');
        Route::post('/skip', [\App\Http\Controllers\Auth\IdMeVerificationController::class, 'skip'])
            ->name('skip');
    });
});
