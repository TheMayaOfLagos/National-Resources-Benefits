                    ...
                    <template #content>
                        <div class="text-center">
                            ...
                            <p class="text-lg font-bold text-gray-900 dark:text-white">{{
                                formatCurrency(stats.total_deposits) }}</p>
                ...
                <Card class="shadow-sm">
                    <template #content>
                        <div class="text-center">
                            ...
                            <p class="text-lg font-bold text-gray-900 dark:text-white">{{
                                formatCurrency(stats.total_withdrawals) }}</p>
                ...
                <Card class="shadow-sm">
                    <template #content>
                        <div class="text-center">
                            ...
                            <p class="text-lg font-bold text-gray-900 dark:text-white">{{
                                formatCurrency(stats.total_transfers_in) }}</p>
                ...
                <Card class="shadow-sm">
                    <template #content>
                        <div class="text-center">
                            ...
                            <p class="text-lg font-bold text-gray-900 dark:text-white">{{
                                formatCurrency(stats.total_transfers_out) }}</p>
                ...
                <Card class="shadow-sm">
                    <template #content>
                        <div class="text-center">
                            ...
                            <p class="text-lg font-bold text-gray-900 dark:text-white">{{ stats.pending_transactions }}
                            </p>
                ...
                <template #content>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                        <Select v-model="filterType" :options="typeOptions" optionLabel="label" optionValue="value"
                            placeholder="Transaction Type" class="w-full" />
                        <Select v-model="filterStatus" :options="statusOptions" optionLabel="label" optionValue="value"
                            placeholder="Status" class="w-full" />
                        <DatePicker v-model="filterDateFrom" placeholder="From Date" dateFormat="yy-mm-dd"
                            class="w-full" />
                        <DatePicker v-model="filterDateTo" placeholder="To Date" dateFormat="yy-mm-dd" class="w-full" />
                        <div class="flex gap-2">
                            <Button label="Filter" icon="pi pi-filter" @click="applyFilters" class="flex-1" />
                            <Button v-if="hasActiveFilters" icon="pi pi-times" severity="secondary"
                                @click="clearFilters" v-tooltip.top="'Clear Filters'" />
                        </div>
                    </div>

                    <!-- Desktop Table -->
                    <div class="hidden md:block">
                        <DataTable :value="transactions.data" stripedRows class="p-datatable-sm" :rows="15">
                                ...
                                <template #body="{ data }">
                                    <div class="flex items-center gap-2">
                                        <div class="w-8 h-8 rounded-full flex items-center justify-center"
                                            ...
                                            <i
                                                :class="['pi text-sm', getTypeConfig(data.type).icon, getTypeConfig(data.type).color]"></i>
                                        </div>
                    ...
                    <div class="md:hidden space-y-3">
                        <div v-for="tx in transactions.data" :key="tx.id"
                            ...
                            <div class="text-right">
                                ...
                                <Tag :severity="getStatusSeverity(tx.status)" :value="tx.status"
                    ...
                    <!-- Pagination -->
                    <Paginator v-if="transactions.total > transactions.per_page" :rows="transactions.per_page"
                        :totalRecords="transactions.total"
                        :first="(transactions.current_page - 1) * transactions.per_page" @page="onPageChange"
                        class="mt-6" />
                </template>
            </Card>
